...
FROM Event,Street,City,Category,Place,(SELECT City1.id AS CityId1 ,Count(Event1.id) As Num_Of_Events
...
WHERE Event.category_id=Category.id and Event.place_id=Place.id and Place.street_id=Street.id and Street.city_id = City.id 
									and City.id=CityMaxEventsTable.CityId1 and Event.start_time>=CURDATE() 
